...
def create_parcel(event):
    ...
    item = {"parcel_id": parcel_id, "sender": body["sender"], "receiver": body["receiver"], "address": body["address"], "customer_email": body["email"], "status": "registered", "created_at": now, "updated_at": now}
...
def list_parcels(event):
    ...
    if status:
        ...
        result = table.query(IndexName="status-index", KeyConditionExpression=Key("status").eq(status))
    else:
        result = table.scan()
...
def update_status(event, parcel_id):
    ...
    if not new_status:
        raise ValueError("Missing field: new_status")
    ...
    now = datetime.now(timezone.utc).isoformat()
    table.update_item(Key={"parcel_id": parcel_id}, UpdateExpression="SET #s = :s, updated_at = :u", ExpressionAttributeNames={"#s": "status"}, ExpressionAttributeValues={":s": new_status, ":u": now})
    sqs.send_message(QueueUrl=QUEUE_URL, MessageBody=json.dumps({"parcel_id": parcel_id, "new_status": new_status, "customer_email": item["customer_email"], "timestamp": now}))
